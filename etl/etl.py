...
def main():
    ...
    # Legacy SQL warehouse load path — no longer used (MinIO Parquet instead).
...
def process_realtor_df(realtor_df, state_abrevs):
    ...
    if len(realtor_df["month_date_yyyymm"].unique()) == 1:
        ...
        realtor_df = pd.merge(
            realtor_df, state_abrevs, how="left", left_on="State", right_on="Code"
        )
        ...
        raise Exception(
            "Realtor dataframe has more than one month_date_yyyymm; expected a single month."
        )
...
def process_census_df(census_df):
    ...
    # edge cases
    # louisiana parishes,
    census_df["county"] = np.where(
        census_df["state"] == "Louisiana",
        census_df["county"].str.replace(" parish", ""),
        np.where(
            census_df["state"] == "Alaska",
            census_df["county"]
            .str.replace(" borough", "")
            .str.replace(" census area", "")
            .str.replace(" municipality", "")
            .str.replace(" city and borough", ""),
            census_df["county"],
        ),
    )
